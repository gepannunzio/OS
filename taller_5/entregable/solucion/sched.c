...
void sched_pause_task(void){
    sched_tasks[current_task].state = TASK_PAUSED;
    ENVIRONMENT->cantTasks--;
}

void sched_resume_task(uint8_t scancode){
    if (scancode >= 2 && scancode <= 5){
        if (sched_tasks[scancode - 2].state != TASK_RUNNABLE){
        sched_tasks[scancode - 2].state = TASK_RUNNABLE;  // 2 = 1, 3 = 2, 4 = 3, 5 = 4
        ENVIRONMENT->cantTasks++;
        }
    }
}
