...
static const uint8_t colores[10] = {
    [0] = C_FG_WHITE         | C_BG_BLACK,		// F1
	[1] = C_FG_RED           | C_BG_GREEN,		// F2
	[2] = C_FG_BLACK         | C_BG_CYAN ,		// F3
	[3] = C_FG_GREEN         | C_BG_BLACK,		// F4
	[4] = C_FG_MAGENTA       | C_BG_BLACK,		// F5
	[5] = C_FG_LIGHT_RED     | C_BG_BLACK,		// F6
	[6] = C_FG_DARK_GREY     | C_BG_BLACK,		// F7
	[7] = C_FG_LIGHT_CYAN    | C_BG_BLACK,		// F8
	[8] = C_FG_LIGHT_MAGENTA | C_BG_BLACK,		// F9
	[9] = C_FG_BROWN         | C_BG_BLACK,		// F10
};
...
static const struct scancode_info scancode_defs[256] = {
	...
	[0x34] = { '.', '>' },
	[0x35] = { '/', '?' },
	[0x36] = { '\0', TOGGLE_SHIFT_IDX }, // RShift
	[0x80 | 0x36] = { '\0', TOGGLE_SHIFT_IDX }, // RShift (UP)
	...
	// main_value = fg y special_value = bg
	...
	[0x57] = { '\0', CLEAR_SCREEN }, // F11 on a 101+ key keyboard
...
static uint8_t curr_attr = C_FG_WHITE | C_BG_BLACK;
static ca under_marker = { .c = '\0', C_FG_CYAN | C_BG_BLACK };
...
static void put_marker() {
	...
	screen[curr_row][curr_column].c = 179;
...
static void change_color(uint8_t scancode) {
	curr_attr = colores[scancode - 0x3b];
}

static void clear_screen(unused uint8_t scancode) {
    screen_draw_box(0, 0, VIDEO_FILS, VIDEO_COLS, 0, 0);
    put_marker();
}

static void backspace(uint8_t scancode) {
	special_scancodes[MOVE_LEFT_IDX](scancode);
	put(' ');
	fix_rowcol();
	put_marker();
}
